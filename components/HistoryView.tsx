...
const HistoryView: React.FC<Props> = ({ history, isSyncing, onUpdate, onSync }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Search & Action Bar */}
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-3 sticky top-[4.5rem] z-20">
        ...
        <div className="flex gap-2">
          <select 
            ...
            className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none"
          >
          ...
          <div className="relative flex-1">
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm pl-9 outline-none focus:ring-2 focus:ring-blue-500"
            />
              ...
              <div className="flex gap-2 items-center">
                <div className="flex gap-1.5">
                  {draw.front.map((num, i) => (
                    <div key={i} className="w-9 h-9 flex items-center justify-center bg-red-50 text-red-600 font-bold rounded-xl border border-red-100 shadow-sm text-sm">
                ...
                <div className="flex gap-1.5">
                  {draw.back.map((num, i) => (
                    <div key={i} className="w-9 h-9 flex items-center justify-center bg-blue-50 text-blue-600 font-bold rounded-xl border border-blue-100 shadow-sm text-sm">
                      {num.toString().padStart(2, '0')}
                    </div>
                  ))}
                </div>
