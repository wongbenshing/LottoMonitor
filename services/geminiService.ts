...
export const getSmartAnalysis = async (history: LottoDraw[], predictedSum?: number, diffVal: number, countVal: number): Promise<AnalysisSummary> => {
  ...
  const response = await ai.models.generateContent({
    ...
    【预测参考值】：
    本系统时间序列分析建议下一期的“前区和值”目标大约为：${sumRef}。
    在推荐号码时，前区5个数字之和不必精确等于此值，允许在 [${sumRef - 2}, ${sumRef + 2}] 范围内浮动。

    【重要参考准则】：
    历史统计发现，任意固定组合在中奖历史中的“最高奖金级别”大概率只有5元(即九等奖)、15元(即八等奖)。请在分析时保持理性，不要过度拟合大奖规律，而是寻找最符合当前概率分布、奇偶平衡及“和值”走势的组合。
    
    1. 识别前区（1-35）和后区（1-12）的冷热号码。
    2. 结合给出的预测和值浮动范围，计算概率上最合理的组合。
