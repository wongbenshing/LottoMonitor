...
def get_back_repeat_index(current_back, prev_back):
    """获取后区重号索引（0-2）"""
    repeat = len([n for n in current_back if n in prev_back])
    return min(repeat, 2)
...
def build_transition_matrices(data):
    ...
    for i in range(len(data) - 1):
        current_back = data[i]['back']
        prev_back = data[i + 1]['back']
        idx = get_back_repeat_index(current_back, prev_back)
