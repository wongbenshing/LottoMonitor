...
def create_range_intervals():
    """创建极差区间定义"""
    intervals = []
    intervals.append({'index': 0, 'min': 4, 'max': 7, 'label': '4-7'})
    ...
    i = 1
    while start <= end:
        max_val = min(start + step - 1, end)
        intervals.append({'index': i, 'min': start, 'max': max_val, 'label': f'{start}-{max_val}'})
        start += step
        i += 1
    
    return intervals

def get_range_interval_index(range_val, intervals):
    """获取极差所在的区间索引"""
    for interval in intervals:
        if range_val >= interval['min'] and range_val <= interval['max']:
...
def build_transition_matrices(data, intervals):
    ...
    range_indices = []
    for draw in data:
        max_val = max(draw['front'])
        min_val = min(draw['front'])
        range_val = max_val - min_val
        idx = get_range_interval_index(range_val, intervals)
        if idx != -1:
    ...
    for i in range(len(range_indices) - 1):
        current = range_indices[i]
        next_idx = range_indices[i + 1]
    ...
    for i in range(len(range_indices) - 2):
        current = range_indices[i]
        prev = range_indices[i + 1]
        next_idx = range_indices[i + 2]
    ...
    for i in range(len(range_indices) - 3):
        current = range_indices[i]
        prev1 = range_indices[i + 1]
        prev2 = range_indices[i + 2]
        next_idx = range_indices[i + 3]
    ...
    for i in range(len(range_indices) - 4):
        current = range_indices[i]
        prev1 = range_indices[i + 1]
        prev2 = range_indices[i + 2]
        prev3 = range_indices[i + 3]
        next_idx = range_indices[i + 4]
...
def precompute_test_windows(data, intervals, test_count=200, train_periods=200):
    ...
    for i in range(len(data) - train_periods - 1):
        ...
        recent_indices = all_range_indices[i+1:i+6]
        actual_idx = all_range_indices[i]
...
def main():
    ...
    intervals = create_range_intervals()
    print(f"\n=== 前区极差区间说明 ===")
    for interval in intervals:
