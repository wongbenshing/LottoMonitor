...
def create_sum_intervals():
    """创建和值区间定义"""
    intervals = []
    intervals.append({'index': 0, 'min': 15, 'max': 34, 'label': '15-34'})
    ...
    intervals.append({'index': i, 'min': 147, 'max': 165, 'label': '147-165'})
    return intervals

def get_sum_interval_index(sum_val, intervals):
    """获取和值所在的区间索引"""
    for interval in intervals:
        if sum_val >= interval['min'] and sum_val <= interval['max']:
...
def build_transition_matrices(data, intervals):
    ...
    for i in range(len(sum_indices) - 4):
        current = sum_indices[i]
        prev1 = sum_indices[i + 1]
        prev2 = sum_indices[i + 2]
        prev3 = sum_indices[i + 3]
        next_idx = sum_indices[i + 4]
...
def precompute_test_windows(data, intervals, test_count=200, train_periods=200):
    ...
    for i in range(len(data) - train_periods - 1):
        ...
        recent_indices = all_sum_indices[i+1:i+6]
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"最佳权重下预测结果与实际结果分布矩阵（前10个区间）")
    print(f"{'='*60}")
    ...
    print(f"{'':<8}", end='')
    for i in range(min(10, len(confusion_matrix))):
        print(f"{i:<10}", end='')
    print()
    
    for i in range(min(10, len(confusion_matrix))):
        print(f"{i:<8}", end='')
        for j in range(min(10, len(confusion_matrix[i]))):
            if i == j:
